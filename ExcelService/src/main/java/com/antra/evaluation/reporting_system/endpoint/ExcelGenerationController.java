...
public class ExcelGenerationController {
    ...
    public ResponseEntity<List<ExcelResponse>> listExcels() {
        ...
        List<ExcelResponse> responseList = fileList.stream().map(file -> {
            ExcelResponse response = new ExcelResponse();
    ...
    public void downloadExcel(@PathVariable String id, HttpServletResponse response) throws IOException {
        ...
        List<ExcelFile> excelFiles = excelService.getExcelList();
        ExcelFile thisFile = null;
        for (ExcelFile ex:excelFiles) {
            if(ex.getFileId() == id){
                thisFile = ex;
            }
        }
        String name = thisFile.getFileName();
        System.out.println("name:*******         "+name);
        StringBuilder sb = new StringBuilder();
        String name1 = sb.append("attachment; filename=").append("\"").append(name).append("\"").toString();
        System.out.println("name1:*******        "+name1);

        System.out.println("name:  ********"+name);
        System.out.println("correct name: *******"+"attachment; filename=\"name_of_excel_file.xls\"");
        ...
        response.setHeader("Content-Disposition", name1);
